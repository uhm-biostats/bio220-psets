...
```{r, fig.align = "center", out.width="50%", fig.cap="Example of using remote sensing to identify likely areas of Rapid ʻŌhiʻa Death (modified from Vaughn et. al (2018)"}
knitr::include_graphics("img/rod.png")
```

Rapid ʻŌhiʻa Death (ROD) is a devastating pathogen affecting native ʻōhiʻa forests in Hawaiʻi. Much work has been done to try to document its spread. [Vaughn et al. (2018)](https://www.mdpi.com/2072-4292/10/4/502) used remote sensing images from 2017 to detect areas of potential ROD. The image above is from the Kohala Mountains on the Island of Hawaiʻi and shows 28.67% ROD. Suppose you were able to re-survey this same area now in 2024 and calculate the percentage of ROD as 34.78%. 

::: {.callout-tip}
## Questions 1--2 refer to the prompt above.

1. True or false: would a binomial test be an appropriate method to test the hypothesis that ROD area has increased over time? [1 point]

    - TRUE
    - FALSE

2. Instead, imagine you did "boots on the ground" survey in 2024 of individual trees and recorded whether each tree was infected with ROD or not. You find that out of 1000 trees you surveyed, 34.8% are infected. In this situation, would a binomial test be an appropriate method to test the hypothesis that ROD prevalence has increased compared to the 28.67% value from 2017 (true or false)? [1 point]
...
Hawaiian *Drosophila* (yes that famous genus of fruit fly) are super diverse and super awesome. They speciate like crazy due to reproductive isolation. When studying reproductive isolation between species, scientists will often allow different species to mate then count the viability of hybrid offspring. Suppose that you are measuring the viability of hybrids eggs laid by a female fruit fly. This female fruit fly will lay eggs over a period of one week, but she needs to be transferred to a fresh vial of food (yeast) every day. After you transfer her to a new vial, you count the number of eggs she laid and record the number of viable eggs. Here are your data:
...
8. Use R code to calculate the probability of observing exactly 10 success from the sampling distribution for $n = 20$ trials with $p = 0.20$ [1 point] Report both your R code and the answer. For example if the question was "Use R code to calculate the sum of 1 plus 1" you would report:
    ...
    # 2
    ```
9. Use R code to find the probability of 10 or more success from the sampling distribution for $n = 20$ trials with $p = 0.20$ [1 point] Report your answer in the same format as for question (8.)
